...
import { CaptureStep, CapturedImages, AnalysisResponse, BrushingHistory, BrushingRecord, ActiveTab } from './types';
...
const App: React.FC = () => {
              ...
              <div className="grid grid-cols-1 gap-4">
                {[
                  ...
                  <div key={idx} className="bg-white/10 p-5 rounded-3xl border border-white/20">
                    ...
                    <p className="text-white text-sm leading-relaxed font-bold">{item.desc}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-8 pb-12">
            {[
              { 
                title: '🦷 치아가 썩었을 때', 
                tips: [
                  '즉시 치과 방문: 통증이 없어도 내부 전이는 매우 빠릅니다.',
                  '과도한 양치 자제: 손상된 부위를 강하게 닦으면 오히려 악화될 수 있어요.',
                  '당분 섭취 즉시 중단: 충치균의 주 먹이인 설탕을 완벽히 차단하세요.'
                ]
              },
              { 
                title: '🔩 임플란트 시 주의사항', 
                tips: [
                  '치간칫솔 사용 필수: 임플란트 주변에 음식물이 끼지 않게 철저히 관리하세요.',
                  '주기적 정기 검진: 신경이 없어 염증이 생겨도 통증을 못 느낄 수 있습니다.',
                  '금연 및 금주: 잇몸뼈와의 결합을 방해하고 주위염의 주원인이 됩니다.'
                ]
              },
              { 
                title: '👄 틀니 시 관리법', 
                tips: [
                  '틀니 전용 세정제: 치약은 연마제가 있어 틀니를 마모시키니 절대 사용 금지!',
                  '수면 시 반드시 제거: 잇몸도 휴식이 필요합니다. 찬물이 담긴 통에 보관하세요.',
                  '부드러운 잇몸 마사지: 부드러운 칫솔로 잇몸을 자극해 혈액순환을 도와주세요.'
                ]
              }
            ].map((section, idx) => (
              <div key={idx} className="bg-white/10 p-8 rounded-[3rem] border border-white/20">
                ...
                <ul className="space-y-4">
                  {section.tips.map((t, i) => (
                    <li key={i} className="flex items-start text-white text-base font-black leading-relaxed">
        ...
        return (
          <div className="flex flex-col items-center px-6 pt-16 pb-32">
            ...
            <div className="text-center space-y-6 mb-12 w-full px-4">
              ...
              <p className="text-white font-black text-xl">당신의 미소를 지키는 천재 AI</p>
            </div>
            
            <div className="w-full space-y-4">
              <button
                onClick={handleStartDiagnosis}
                className="group w-full py-7 bg-gradient-to-br from-red-600 to-red-800 text-white rounded-[2.5rem] font-black text-2xl shadow-[0_15px_40px_-10px_rgba(220,38,38,0.8)] active:scale-95 transition-all transform flex items-center justify-center space-x-4 border border-white/20 overflow-hidden"
              >
                <span>충격받을 준비하기</span>
                <span className="text-4xl inline-block animate-shiver">🥶</span>
              </button>
              <p className="text-center text-white/80 font-black text-xs uppercase tracking-widest">
                ※ ai사용으로 단순 참조용입니다!
              </p>
            </div>
            ...
            <div className="mt-16 p-8 bg-white/10 backdrop-blur-xl rounded-[3rem] border border-white/20 w-full shadow-2xl relative z-10">
              ...
              <p className="text-white text-base font-black leading-relaxed">밤 양치 전 치실 사용은 수면 중 세균 번식을 90% 차단합니다. 오늘 밤 로직이와 약속해요!</p>
            </div>
          </div>
        );
      case 'history':
      ...
      case 'tips':
        return (
          <div className="p-6 space-y-8 animate-in slide-in-from-right-4 duration-500 pb-48">
            ...
            <div className="flex space-x-2 bg-white/10 p-2 rounded-full border border-white/20">
              {[
                { id: 'food', label: '음식', emoji: '🍎' },
                { id: 'care', label: '관리', emoji: '🏥' },
                { id: 'glossary', label: '용어정리', emoji: '📚' }
              ].map((cat) => (
                <button
                  key={cat.id}
                  onClick={() => { audioService.playTap(); setTipCategory(cat.id as any); }}
                  className={`flex-1 flex items-center justify-center space-x-2 py-3 px-4 rounded-full font-[1000] text-sm transition-all ${
                    tipCategory === cat.id ? 'bg-white text-black shadow-lg' : 'text-white hover:bg-white/10'
                  }`}
                >
                  <span>{cat.emoji}</span>
                  <span>{cat.label}</span>
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col font-sans selection:bg-blue-500/30">
      <style>{`
        @keyframes sway {
          0%, 100% { transform: translateX(-10px) translateY(0) rotate(-5deg); }
          50% { transform: translateX(10px) translateY(-10px) rotate(5deg); }
        }
        @keyframes zoomInMouth {
          0% { transform: scale(0.1); opacity: 0; }
          30% { transform: scale(1.5); opacity: 1; }
          80% { transform: scale(20); opacity: 1; }
          100% { transform: scale(30); opacity: 0; }
        }
        @keyframes popText {
          0% { transform: scale(0) rotate(-20deg); opacity: 0; }
          50% { transform: scale(1.8) rotate(10deg); opacity: 1; }
      ...
      <header className="bg-black/80 backdrop-blur-2xl sticky top-0 z-50 border-b border-white/20 px-6 py-5 flex items-center justify-between">
        ...
        {step === 'RESULT' && (
          <button onClick={reset} className="text-sm font-black text-white bg-blue-600 border border-white/20 px-5 py-2.5 rounded-full uppercase tracking-widest shadow-lg active:scale-90 transition-transform">다시 진단</button>
        )}
      </header>
      ...
      <main className="flex-1 overflow-y-auto relative bg-mesh">
        ...
        {step === 'TRANSITION' && (
          <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center overflow-hidden">
            <div className="animate-zoom-mouth text-[180px] pointer-events-none drop-shadow-[0_0_80px_rgba(255,255,255,0.3)]">👄</div>
            <div className="animate-pop-text absolute text-8xl font-[1000] text-blue-500 drop-shadow-[0_0_40px_rgba(59,130,246,0.8)]">Ready!</div>
          </div>
        )}
        ...
        {step === 'LOWER' && (
          <CameraCapture 
            ...
            onCapture={(img) => {
              audioService.playCheck();
              const newImages = { ...images, lower: img };
              setImages(newImages);
              setStep('PREVIEW');
            }} 
            onBack={() => setStep('UPPER')}
          />
        )}

        {step === 'PREVIEW' && (
          <div className="max-w-md mx-auto p-8 space-y-10 animate-in fade-in duration-500 pb-48">
            <h2 className="text-4xl font-black text-white tracking-tight">촬영 확인 <span className="text-blue-400">📸</span></h2>
            <div className="grid grid-cols-1 gap-8">
              {[
                { label: 'Upper Jaw (상악)', img: images.upper, step: 'UPPER' },
                { label: 'Lower Jaw (하악)', img: images.lower, step: 'LOWER' }
              ].map((item, i) => (
                <div key={i} className="space-y-4">
                  <div className="flex items-center justify-between px-2">
                    <label className="text-xs font-black text-white uppercase tracking-widest">{item.label}</label>
                    <button onClick={() => { audioService.playTap(); setStep(item.step as CaptureStep); }} className="text-blue-400 text-sm font-black border-b border-blue-400/50">RETAKE</button>
                  </div>
                  <div className="relative rounded-[3rem] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.8)] border-2 border-white/20 aspect-[4/3]">
            ...
            {error && <p className="text-white text-sm font-black text-center bg-red-600/50 py-4 rounded-3xl border border-white/20">{error}</p>}
            <button
              onClick={() => { audioService.playTap(); startAnalysis(images); }}
              className="w-full py-7 bg-white text-black rounded-[2.5rem] font-black text-2xl shadow-[0_0_50px_rgba(255,255,255,0.2)] active:scale-95 transition-all"
            >
              로직이 분석 시작! ✨
            </button>
          </div>
        ...
        {step === 'ANALYZING' && (
          <div className="flex flex-col items-center justify-center h-full min-h-[75vh] space-y-10 px-10 text-center">
            <div className="relative">
              <div className="w-40 h-40 border-[12px] border-white/10 border-t-blue-600 rounded-full animate-spin"></div>
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-7xl animate-pulse">🦷</span>
              </div>
            </div>
            <div className="space-y-4">
              <h3 className="text-4xl font-[1000] text-white tracking-tight">정밀 스캔 중...</h3>
              <p className="text-white font-black text-lg opacity-100">데이터 엔진이 치아와 잇몸을 분석하고 있습니다</p>
            </div>
          </div>
        )}
      ...
      {step === 'INITIAL' && (
        <nav className="fixed bottom-0 left-0 right-0 bg-black/95 backdrop-blur-3xl border-t border-white/20 px-8 py-5 pb-12 z-50">
          <div className="max-w-xl mx-auto flex justify-between items-center">
            {[
              ...
              <button
                ...
                className={`flex flex-col items-center space-y-2 px-6 py-2 rounded-3xl transition-all duration-300 ${
                  activeTab === tab.id ? 'text-white bg-white/20 scale-110 shadow-[0_0_20px_rgba(255,255,255,0.1)]' : 'text-white/50 hover:text-white/80'
                }`}
              >
                <span className={`text-2xl transition-transform ${activeTab === tab.id ? 'scale-125' : 'grayscale-0 opacity-50'}`}>{tab.icon}</span>
                <span className={`text-[11px] font-[1000] uppercase tracking-widest ${activeTab === tab.id ? 'text-white' : 'text-white'}`}>
                  {tab.label}
                </span>
              </button>
            ))}
